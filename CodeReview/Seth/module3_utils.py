...
def plt_tide(pt_tide, time_index, start_date, end_date):
    """
        ...
        OUTPUTS: A matplotlib subplot with tidal elevations,
                 and a vertical marker at the location specified
                in time_index
    """
...
def plt_ferry_and_tide(ferryQc, pt_tide, crossing_index,
                       start_date, end_date):
    """ plots Port Townsend tidal elevations and ferry crossing
        data on two subplots.

        INPUTS
            ferryQc:     xarray data set with quality controlled ferry
                         data
            pt_tide:     port townsend tidal elevations as a
                         pandas DataFrame
            cross_index: integer of ferry crossing to be displayed
            start_date:  string, start of tidal elevation time series
                         to be plotted
            end_date:    string, end of tidal elevation time series
                         to be plotted
        OUTPUTS
    ...
    min_xing = min(
        ferry_subsample.xing_num[ferry_subsample.xing_num != -9].values)
    max_xing = max(
        ferry_subsample.xing_num[ferry_subsample.xing_num != -9].values)
    ...
    if (crossing_index > valid_xings.size):
        ...
        print(str(valid_xings.size) +
              ' number of crossings for the chosen date range')
        return
    ...
    if not np.all(
           np.isnan(ferry_subsample.Horizontal_speed[plt_index].values)):
